...
public class RenderDataFactory {
    ...
    private long encodeFluidStep(long quad, int lowerHeight, int voxelIndex) {
        ...
        //Bits 42..45 double as the "quad carries a fluid shape payload" flag the shader keys the quad
        //size off (quadHasFluidShape()): while non-zero the shader treats the quad as a one-voxel shaped
        //fluid quad. This method runs on MERGED fluid runs, so writing the step height there made every
        //merged run render as a one-voxel patch and the distant water disintegrated into a moire grid.
        //The biome is still written; the payload bits are left clear so merged fluid quads keep their size.
        quad &= ~((0xFL << 42) | (1L << 63));
            ...
            for (int other = 0; other < 32; other++) {
                ...
                while (msk != 0) {
...

                    {
                        ...
                        //The fluid TOP faces used to be skipped here and generated by the shaped "direct"
                        //path instead. That path is disabled (its corner-height payload caused the
                        //distant-water moire), so the top faces come from this ordinary generator like
                        //the reference ports do.
            ...
            for (int other = 0; other < 32; other++) {
                ...
                while (msk != 0) {
...

                    {
                        ...
                        //The shaped "direct" generators used to own the water tops and the Z side walls; they
                        //are skipped now, so these faces come from the plain path like the reference ports.
    ...
    private void generateFluidFaces(int unavailableNeighborMsk) {
        ...
        //The three "direct" fluid generators (DirectHorizontal, DirectOuterHorizontal, DirectFluidTops)
        //carry the corner-height shape payload in the quad's size bits; that path produced the moire
        //grid on distant water and is skipped. The remaining generators emit plain merged quads,
        //exactly like the reference ports whose water renders correctly.
